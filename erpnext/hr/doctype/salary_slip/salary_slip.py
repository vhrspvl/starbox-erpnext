...
class SalarySlip(TransactionBase):
    ...
    def get_employee_attendance(self):
        ...
        employee_attendance = frappe.db.sql("""select name,attendance_date from `tabAttendance` where \
            docstatus = 1 and status = 'Present' and employee= %s and attendance_date between %s and %s""", (self.employee, self.start_date, self.end_date), as_dict=1)
        ...
        if not self.salary_slip_based_on_timesheet:
            ...
            for data in self.timesheets:
                if frappe.db.get_value('Timesheet', data.time_sheet, 'status') == 'Payrolled':
                    frappe.throw(_("Salary Slip of employee {0} already created for time sheet {1}").format(
    ...
    def sum_components(self, component_type, total_field):
        ...
        for d in self.get(component_type):
            if (self.salary_structure and cint(d.depends_on_lwp)):
                d.amount = rounded(
    ...
    def calculate_net_pay(self):
        ...
        disable_rounded_total = cint(frappe.db.get_value(
            "Global Defaults", None, "disable_rounded_total"))
