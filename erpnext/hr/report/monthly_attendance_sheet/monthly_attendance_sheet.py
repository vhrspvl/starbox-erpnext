...
def execute(filters=None):
    ...
    for emp in sorted(att_map):
        ...
        row = [emp, emp_det.biometric_id, emp_det.employee_name, emp_det.branch, emp_det.department, emp_det.employment_type, emp_det.contractor, emp_det.designation,
               emp_det.company]

        total_p = total_a = total_l = 0.0
        for day in range(filters["total_days_in_month"]):
            status = att_map.get(emp).get(day + 1, "None")
            status_map = {"Present": "P", "On Duty": "OD", "Late": "P", "Absent": "A",
                          "Half Day": "HD", "On Leave": "L", "None": "", "Holiday": "<b>H</b>"}
            ...
            if status == "Present" or status == "Late" or status == "On Duty":
                total_p += 1
            elif status == "Absent":
                total_a += 1
...
def get_columns(filters):
    columns = [
        _("Employee") + ":Link/Employee:120", _("Employee ID") + ":Datas:120", _("Employee Name") +
        "::140", _("Branch") + ":Link/Branch:120",
        _("Department") + ":Link/Department:120", _("Employment Type") + ":Link/Employment Type:120", _(
            "Contractor") + ":Link/Contractor:120", _("Designation") + ":Link/Designation:120",
...
def get_employee_details():
    ...
    for d in frappe.db.sql("""select name,biometric_id, employee_name, designation, department,employment_type,contractor, branch, company,
